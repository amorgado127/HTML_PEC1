...
window.onload = function () {
  ...
  const libroDatos2 = document.getElementsByClassName("libro")[0];
  

  libros.forEach((libro,index) => {
    libroEnlace=libroDatos.getElementsByClassName("enlace-detalle")[0];
    libroPortada_th=libroDatos.getElementsByClassName("img-portada-th")[0];
    libroTitulo=libroDatos.getElementsByClassName("titulo")[0];
    libroAutor=libroDatos.getElementsByClassName("autor")[0];
    
    libroEnlace="./detalle.html?isbn="+libro.isbn;
    libroPortada_th.src=libro.imagenPequeña;
    libroTitulo.textContent=libro.titulo
    libroAutor.textContent=libro.autor;

    if(index>0){ // El primero ya está definido en el HTML, el resto de libros hay que añadirlos.
      listaLibros.appendChild(libroDatos);
    }
